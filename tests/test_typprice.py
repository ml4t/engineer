"""
Test Typical Price (TYPPRICE) indicator implementation.

TYPPRICE = (High + Low + Close) / 3
The typical price is the average of high, low, and close (excluding open).
"""
...
from ml4t.engineer.features.price_transform import typprice


class TestTypicalPrice:
    """Test Typical Price indicator."""
    ...
    def test_typprice_accuracy(self, price_data):
        """Test TYPPRICE matches TA-Lib exactly."""
    ...
    def test_typprice_polars(self, price_data):
        """Test TYPPRICE with Polars expressions."""
        ...
        assert_allclose(
            result_df["typprice"].to_numpy(),
            expected,
            rtol=1e-10,
            equal_nan=True,
        )

    def test_typprice_edge_cases(self):
        """Test TYPPRICE with edge cases."""
        # Case 1: Single value
        high = np.array([101.0])
        low = np.array([99.0])
        ...
        low = np.array([99.0, np.nan, 101.0])
        close = np.array([100.5, 101.0, 102.5])

        result = typprice(high, low, close)
        # Second value should be NaN due to NaN in low
        assert ~np.isnan(result[0])
        assert np.isnan(result[1])
        assert ~np.isnan(result[2])

    def test_typprice_calculation_manual(self):
        """Test TYPPRICE calculation manually."""
        ...
        expected = np.array(
            [
                (102.0 + 99.0 + 101.0) / 3.0,  # 100.666...
                (103.0 + 100.0 + 102.0) / 3.0,  # 101.666...
                (104.0 + 101.0 + 103.0) / 3.0,  # 102.666...
            ],
        )

        assert_allclose(result, expected, rtol=1e-10)

    def test_typprice_parameter_validation(self):
    ...
    def test_typprice_crypto_accuracy(self, crypto_data_small):
        """Test TYPPRICE accuracy on real crypto data."""
    ...
    def test_typprice_performance(self, crypto_data, performance_threshold, warmup_jit):
        ...
        warmup_jit(typprice, high[:100], low[:100], close[:100])

        import time

        # Our implementation (warmed up)
        start = time.perf_counter()
        _ = typprice(high, low, close)
        our_time = time.perf_counter() - start

        # TA-Lib
        start = time.perf_counter()
        _ = talib.TYPPRICE(high, low, close)
        talib_time = time.perf_counter() - start

        print(f"\nTYPPRICE Performance ({len(high):,} rows of crypto data):")
